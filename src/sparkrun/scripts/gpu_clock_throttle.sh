...
sudo -n nvidia-smi {clock_args} >/dev/null
echo "{result_label}"

case "{unit_action}" in
install)
    sudo -n tee "$UNIT_FILE" > /dev/null << UNIT_EOF
[Unit]
...
UNIT_EOF
    ...
    sudo -n systemctl enable "$UNIT_NAME" >/dev/null 2>&1
    echo "UNIT: installed, enabled at boot"
    ;;
remove)
    if [ -f "$UNIT_FILE" ]; then
        sudo -n systemctl disable "$UNIT_NAME" >/dev/null 2>&1 || true
